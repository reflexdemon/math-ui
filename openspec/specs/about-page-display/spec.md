...
### Requirement: About page uses application standard theming

The about page SHALL use PrimeVue CSS variables for theming consistent with the rest of the application.

#### Scenario: Light mode theming

- **WHEN** user views the about page in light mode
- **THEN** the page uses `var(--p-surface-100)` for background
- **AND** uses `var(--p-surface-900)` for text color

#### Scenario: Dark mode theming

- **WHEN** user views the about page in dark mode (`.p-dark` class present)
...
- **WHEN** the about page is viewed on any screen size
